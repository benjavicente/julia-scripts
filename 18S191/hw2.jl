...
student = (name = "Benjamín Vicente Goecke", kerberos_id = "benjavicente")
...
$(1029 - 687) or $(round((1029 - 687)/1029 * 100))% \
With vcat the **two arrays** are obtained from the image, to use vcat
and then the **new array is added** to the image.
Without vcat the **two arrays are directly given to the new image**, reducing
the number of operations from **3 to 2 ( $(round((3-2)/3 * 100))% diference)**
"""
...
1026 allocations! with views, no copies of arrays are made in the loop
"""
...
function greedy_seam(energies, starting_pixel::Int)
	...
	@views for i ∈ 2:m
		lower = clamp(seam[i-1] - 1, 1, n)
		upper = clamp(seam[i-1] + 1, 1, n)
		(minval, index) = findmin(energies[i, lower:upper])
		seam[i] = lower + index - 1
	end
	seam
end

# ╔═╡ aebd7250-4b98-11eb-14b0-737f21ee36e7
@benchmark greedy_seam(energy(img), 2)
...
function least_energy(energies, i, j)
	...
	i == n && return energies[i, j]

	lower = clamp(j - 1, 1, n)
	upper = clamp(j + 1, 1, n)

	minval = minimum(least_energy(energies, i + 1, j′) for j′=lower:upper)
	return energies[i, j] + minval
end
...
function recursive_seam(energies, starting_pixel)
	...
	for i=2:m
		lower = clamp(seam[i-1] - 1, 1, n)
		upper = clamp(seam[i-1] + 1, 1, n)
		(_, index) = findmin([least_energy(energies, i, j′) for j′=lower:upper])
		seam[i] = lower + index - 1
	end
	seam
end

# ╔═╡ 72e3ebd0-4ba5-11eb-1557-4d6ca0d1665f
@bind partida Slider(1:8)
...
function least_energy_matrix(energies)
	...
	for i ∈ m-1:-1:1
		for j ∈ 1:n
			lower = max(j - 1, 1)
			upper = min(j + 1, n)
			minval, index = findmin([least_energies[i+1, j′] for j′=lower:upper])
			least_energies[i, j] += minval
		end
	end
	least_energies
end

# ╔═╡ 61a6821e-4bba-11eb-24bb-7b7d06d62c64
0.2* least_energy_matrix(energy(img)) .|> RGB 

# ╔═╡ f1eac33e-4bbb-11eb-362c-53e3dc6b2370
img
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	for i ∈ 2:m
		lower = max(seam[i-1] - 1, 1)
		upper = min(seam[i-1] + 1, n)
		minval, index = findmin([least_energies[i, j′] for j′=lower:upper])
		seam[i] = lower + index - 1
	end
...
# ╔═╡ 21b4d97c-4ba5-11eb-007d-13b5db38fa07
recursive_seam(energy(pika), partida)

# ╔═╡ d88bc272-f392-11ea-0efd-15e0e2b2cd4e
if shrink_recursive
	recursive_carved = shrink_n(pika, 1, recursive_seam)
