...
unused_letters = let
	[letter for letter ∈ alphabet if sample_freqs[index_of_letter(letter)] <= 0]
end
...
double_letters = let
	alphabet[argmax(findmax(diag(sample_freq_matrix)))]
end
...
begin
	sum_rows = sum.(sample_freq_matrix)
	sum_columns = sum.(sample_freq_matrix')
	row_col_answer = md"""
...
function completions_cache(grams)
	cache = Dict()
	for g ∈ grams
		if g[1:end-1] ∈ keys(cache)
			push!(cache[g[1:end-1]], g[end])
		else
			cache[g[1:end-1]] = [g[end]]
		end
	end
	cache
end
