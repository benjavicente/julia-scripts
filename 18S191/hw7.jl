...
student = (name = "benja Vicente", kerberos_id = "benjavicente")
...
function intersection_distance(photon::Photon, wall::Wall)
	- (photon.p - wall.position) ⋅ wall.normal / (photon.l ⋅ wall.normal)
end
...
function intersection(photon::Photon, wall::Wall; ϵ=1e-3)
	...
	if D > ϵ
		Intersection(wall, D, D * photon.l + photon.p)
	else
		Miss()
	end
end
...
let
	l      = [1, 1]
	normal = [1, 0]
	@assert reflect(l, normal) == [-1, 1] "reflect is not working!"
end
...
function intersection(photon::Photon, sphere::Sphere; ϵ=1e-3)
	a = photon.l ⋅ photon.l
	b = 2photon.l ⋅ (photon.p - sphere.center)
	...
	elseif d == 0
		...
		t = -(b + √d) / 2a
		if t > ϵ
			Intersection(sphere, t, t * photon.l + photon.p)
		else
			if t < ϵ
				Miss()
			else
...
function closest_hit(photon::Photon, objects::Vector{<:Object})
	minimum(intersection(photon, obj) for obj in objects)
end
...
# ╔═╡ 11425c38-4f81-11eb-2e28-cfbab6c91c0c
let
	test_sphere = Sphere(
		philip.p + [0, 2],
		2,
...
# ╔═╡ 11e32d30-4f85-11eb-357c-1bbea64ebd91
philip.p * (1 - philip.l ⋅ philip.p)

# ╔═╡ f9ffd9d2-4f81-11eb-0e50-d73d05f10f40
let
	test_sphere = Sphere(
		[3, -1],
		2,
...
function interact(photon::Photon, hit::Intersection{Sphere})
	if photon.ior == 1.0
		Photon(
			...
			refract(
				photon.l,
				sphere_normal_at(photon.p, hit.object),
				1.0,
				hit.object.ior
			),
			hit.object.ior
...
function trace(photon::Photon, scene::Vector{<:Object}, N)
	accumulate(step_ray, scene for i=1:N; init=photon)
end
...
test_lens_photon = Photon([0,0], [1,0.8], 1.0)
